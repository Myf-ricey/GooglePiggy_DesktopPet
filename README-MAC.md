...
这是 GooglePiggy 猪猪桌宠的原生 macOS 版。当前发布版本为 `0.2.3`，保留
Windows 版的完整互动：
...
- 优先下载同时支持两类处理器的 `GooglePiggy-macos-universal`；
...
3. 单击猪猪，应播放一次躺平动画。
4. 拖动猪猪，应播放左拱动画并跟随鼠标。
5. 右键打开菜单后点击空白处取消，猪猪应继续显示。
6. 当 Codex 请求权限时，说明文字应自动换行，并可通过“允许/拒绝”回传。
